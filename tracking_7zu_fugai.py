...
def tracking(input_video, output_video):
    ...
    obstructed_blocks_dir = os.path.join(output_image_folder, 'obstructed_blocks')
    non_obstructed_blocks_dir = os.path.join(output_image_folder, 'non_obstructed_blocks')
    ...
    for d in [output_image_folder, obstructed_blocks_dir, non_obstructed_blocks_dir, bg_dir, inflate_dir,result_dir]:
    ...
    while True:
        ...
        tracklist = [ (t[0], t[1], t[2], t[3]+1, t[4],t[5]) for t in tracklist ]

        if not tracklist:
            for (xy, area) in xy_areas:
                tracklist.append((xy, (0, 0), 1, 0, area,0))
                same_blocks_list=[[0]]
        else:
            # if frame_count==65:
            #     cv2.waitKey(0)
            ...
            for (xy, current_area) in xy_areas:
                ...
                for i, trackitem in enumerate(tracklist):
                    ...
                    if current_dis<min_dis and trackitem[3]<6:
                        min_dis=current_dis
                        min_index = i
                    if current_dis < min(45, dis)and trackitem[3]<6 and xy[0]<480:
                        dis = current_dis
                        closest = i
                # cv2.waitKey(0)
                ...
                if dis == 1000:
                    if xy[0]<460:
                        ...
                        for i, linked_list in enumerate(same_blocks_list):
                            if isinstance(linked_list, list) and min_index in linked_list:
                                motion = (xy[0] - tracklist[closest][0][0], 
                                    xy[1] - tracklist[closest][0][1])
                                tracklist.append((xy, motion, 1, 0, current_area,i))
                                color_index=i
                                same_blocks_list[i].append(len(tracklist)-1)  
                        cv2.circle(frame, (round(xy[0]), round(xy[1])), 3, 
                            trackcolor[same_blocks_list[color_index][0] % len(trackcolor)], 2)
                    
                    else:
                        tracklist.append((xy, (0, 0), 1, 0, current_area,len(tracklist)))
                        same_blocks_list.append([len(tracklist)-1])
                        cv2.circle(frame, (round(xy[0]), round(xy[1])), 3, 
                    ...
                    motion = (xy[0] - tracklist[closest][0][0], 
                            xy[1] - tracklist[closest][0][1])
                    tracklist[closest] = (xy, motion, tracklist[closest][2]+1, tracklist[closest][3]-1, current_area,tracklist[closest][5])
                    cv2.circle(frame, (round(xy[0]), round(xy[1])), 3, 
                ...
                cv2.imwrite(os.path.join(result_dir, 
                                    f"{tracklist[closest][5]}/{frame_count}._{xy}.jpg"), 
                            frame[y1:y2, x1:x2])
        occlusion_reasons = []
        if avg_area > 50 and max_area > avg_area * OCCLUSION_AREA_RATIO:
            occlusion_reasons.append("LargeArea")

        save_dir = obstructed_blocks_dir# if occlusion_reasons else non_obstructed_blocks_dir
        ...
        for i,trackitem in enumerate(tracklist):
            if trackitem[3]>0:
                tracklist[i] = (tracklist[i][0], tracklist[i][1], 0, tracklist[i][3], tracklist[i][4],tracklist[i][5])
        for i, trackitem in enumerate(tracklist):
    ...
    for i in range(len(tracklist)):
        print(tracklist[i])
    logger.info(f"same_blocks_list={same_blocks_list}")
