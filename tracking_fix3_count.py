        ...
        if not tracklist:
            ...
            for (xy, current_area) in xy_areas:
                ...
                for i, trackitem in enumerate(tracklist):
                    ...
                    if current_dis < min(45, dis)and trackitem[3]<6 and xy[0]<484:
                        dis = current_dis
                        closest = i
                # cv2.waitKey(0)
                #判断这个距离超过阈值的积木是否是新积木
                if dis == 1000:
                    if xy[0]<460:#不是新下落的积木，可能前一帧有遮挡
                        color_index=0
                        closest=min_index
                        for i, linked_list in enumerate(same_blocks_list):
                            if min_index in linked_list.track_indexes:
                                ...
                                tracklist.append((xy, motion, 1, 0, current_area,min_index))                
                                color_index=i
                                linked_list.track_indexes.append(len(tracklist)-1)  
                        cv2.circle(frame, (round(xy[0]), round(xy[1])), 3, 
                            trackcolor[same_blocks_list[color_index].track_indexes[0] % len(trackcolor)], 2)
