...
def tracking(input_video, output_video):
    ...
    while True:
        ...
        if fgmask is None:
            ...
            continue
        fgmask = (fgmask * 255).astype(np.uint8)
        # 显示前景掩码
        cv2.imshow('f0', fgmask)
        # 去除噪声（可选）
        ...
        # 确保fgmask不是None，并且是一个有效的图像数组
        if fgmask is not None:
            # 将fgmask转换为8位无符号整数类型，并乘以255（如果它是浮点数或需要缩放的话）
            fgmask = (fgmask * 255).astype(np.uint8)
            # 显示图像
            cv2.imshow('F', fgmask)
        else:
            print("Warning: fgmask is None and cannot be displayed.")
        ...
        for contour in contours:
            if cv2.contourArea(contour) > 50:  # 过滤掉太小的轮廓
        ...
        # 检查是否需要保存积木图片
        for i, trackitem in enumerate(tracklist):
            if trackitem[2] > 7 and i not in saved_tracks:
